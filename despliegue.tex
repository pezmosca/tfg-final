...
\\
Con tal despliegue se puede soportar que caigan dos máquinas y aún el módulo podrá continuar haciendo su trabajo. Si ninguna máquina está caída se tendrán 24 cores que podrán soportar conexiones de hasta 30 Gbps de tráfico agregado.
...
Este módulo ya está desplegado en la empresa, por lo que el despliegue que se va a hacer es exclusivo para la demostración. Por un lado se ha desplegar un cluster de Elastic Search y una máquina con Kibana, y por otro una máquina con JIRA.

Para el despliegue del cluster de Elastic Search se va a utilizar máquinas parecidas a las que utiliza la empresa. La máquina escogida es una r4.xlarge, en la tabla \ref{tabla:kafkazookeeper} se puede ver sus recursos hardware. Para el despligue de Kibana se va a utilizar una t2.micro ya que no consume grandes recursos, en la tabla \ref{tabla:t2.micro} se puede ver sus recursos hardware.
\\
Para el despligue de JIRA se va a utilizar una t2.small ya que JIRA como mínimo requiere 2 GiB de RAM para funcionar fluido. En la tabla \ref{tabla:t2.small} se puede ver sus recursos hardware.
